...
def inference(args, model, iters=32, mixed_prec=False):
    def load_K(camera_path):
        ...
        for line in lines:
            if not line.startswith('#'):
                ...
                print(elem_list)
                K = np.array([[float(elem_list[4]), 0, float(elem_list[6])],
                              [0, float(elem_list[5]), float(elem_list[7])],
                              [0, 0, 1]])
                K = torch.from_numpy(K).float().cuda(args.device)[None]
        return K
    ...
    """ Peform inference using a single pair of images """
    model.eval()
    left_imgs = [os.path.join(args.left_img_dir,x) for x in sorted(os.listdir(args.left_img_dir))]
    right_imgs = [os.path.join(args.right_img_dir,x) for x in sorted(os.listdir(args.right_img_dir))]
    pose_list = np.load(args.pose_path)
    params = {}
    baseline = torch.tensor(0.02).float().cuda(args.device)[None]
    K_raw = load_K(args.camera_path)
    for idx, (image1, image2) in enumerate(zip(left_imgs, right_imgs)):
        frame_id = image1.split('/')[-1].split('.')[0]
        image1, image2 = load(image1, image2)
        ...
        padder = InputPadder(image1.shape, divis_by=32)
        [image1, image2], K = padder.pad(image1, image2, K=K_raw)

        with autocast(enabled=mixed_prec):
            testing_output = model(image1, image2, iters=iters, test_mode=True, params=None if idx == 0 else params)
        ...
        disp_pr = pseudoColorMap(disp_pr, vmin=0, vmax=48, cmap='jet', kitti_style=False)
        os.makedirs(args.output_dir, exist_ok=True)
        skimage.io.imsave(os.path.join(args.output_dir,frame_id+'.png'), disp_pr)
        print(f"Saved the disparity map to {os.path.join(args.output_dir,frame_id+'.png')}")
...
if __name__ == '__main__':
    ...
    # inference
    parser.add_argument('--left_img_dir', type=str, default=None, help="the directory of left images")
    parser.add_argument('--right_img_dir', type=str, default=None, help="the directory of right images")
    parser.add_argument('--output_dir', type=str, default=None, help="the output directory of disparity maps")
    parser.add_argument('--pose_path', type=str, default=None, help="the path of pose file")
    parser.add_argument('--camera_path', type=str, default=None, help="the path of camera file")
    ...
    inference(args, model, iters=args.valid_iters, mixed_prec=use_mixed_precision)